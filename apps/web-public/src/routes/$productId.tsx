...
import {
	...
	useProductHistory,
} from "@web-public/api/products/queries";
import {
	Alert,
	AlertDescription,
	AlertTitle,
} from "@web-public/components/ui/alert";
...
import {
	...
	ChartTooltip,
	ChartTooltipContent,
} from "@web-public/components/ui/chart";
import { Checkbox } from "@web-public/components/ui/checkbox";
import {
	DropdownMenu,
	...
	DropdownMenuTrigger,
} from "@web-public/components/ui/dropdown-menu";
import {
	Select,
	SelectContent,
	SelectGroup,
	...
	SelectTrigger,
	SelectValue,
} from "@web-public/components/ui/select";
import { Spinner } from "@web-public/components/ui/spinner";
import {
	Table,
	TableBody,
	TableCell,
	...
	TableHeader,
	TableRow,
} from "@web-public/components/ui/table";
import {
	Tabs,
	TabsContent,
	TabsList,
	TabsTrigger,
} from "@web-public/components/ui/tabs";
import { getCountryChartTheme } from "@web-public/lib/chart-colors";
import { currencyCodes } from "@web-public/lib/currencies";
		...
		<div className="flex min-h-[40dvh] items-center justify-center">
			<Card className="w-full max-w-lg border-destructive/30 bg-destructive/5">
				<CardHeader>
					...
					<CardDescription>
						This product could not be loaded. It may have been removed or the
...
function RouteComponent() {
	...
	return (
		<div className="space-y-6">
			...
			<div className="relative grid gap-4 rounded-2xl border bg-card p-4 sm:grid-cols-[auto_1fr_auto]">
				<div className="space-y-1.5">
					<span className="font-medium text-muted-foreground text-xs">
						Display currency
					</span>
					<Select
						items={currencyCodes.map((currencyCode) => ({
							label: currencyCode,
							value: currencyCode,
						}))}
				...
				<div className="space-y-1.5">
					...
					<div className="flex h-9 items-center gap-2 rounded-md border bg-background px-3 text-sm">
						<Checkbox
							...
							onCheckedChange={setIncludeEuAverage}
						/>
						<button
							type="button"
							className="text-left"
							onClick={() => setIncludeEuAverage((value) => !value)}
						>
							Show EU average
						</button>
					</div>
					...
					{currentPricesError ? (
						...
						<Tabs defaultValue="list" className="gap-4">
							<div className="flex flex-wrap items-center justify-between gap-3">
								...
								<p className="text-muted-foreground text-xs">
									{currentPrices.data.length} prices
								</p>
							</div>
								...
								<Table>
									...
									<TableBody>
										{currentPrices.data.map((row) => (
											<TableRow key={row.itemId} className="border-none">
												<TableCell className="max-w-48 truncate font-medium">
													{row.itemName || row.itemUrl}
												</TableCell>
												<TableCell>{row.siteName}</TableCell>
												<TableCell>
													<Badge variant="outline">{row.countryName}</Badge>
												</TableCell>
